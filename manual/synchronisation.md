...
It is the **Sync** step of the workflow, after the video and the log are open.

![Sync step](../images/Screenshot_3.png)
...
The other two methods measure offset and drift by comparing how fast the camera rotates with the gyro in the blackbox.
They pick several short windows across the flight, match each against the whole log and fit one straight line through them.
They need a flight with some rotation; smooth cruising gives weak matches.
...
1. In Gyroflow open the video and save the project next to it (`<video>.gyroflow`); the app finds it automatically, otherwise click **Open…** in the **Gyroflow project** card and pick it.
2. Choose **Gyroflow**, check that the three gyro columns are filled (`gyroADC[0..2]` are found automatically), click **Analyse**.
...
4. **Apply** writes the offset and drift. Verify by scrubbing to a fast manoeuvre; fine-tune in **Manual** if needed.

The same steps apply to *Video motion*, only without the Gyroflow file.
