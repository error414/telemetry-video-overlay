  ...
  return (
    <nav className="stepper" aria-label="Workflow steps">
      {steps.map((s, i) => {
        ...
        const disabled = (locked && !active) || (s.disabled && !active);
        return (
          <React.Fragment key={s.id}>
            {i > 0 && <span className={'step-line' + (steps[i - 1].done ? ' done' : '')} />}
            <button className={'step' + (active ? ' active' : '') + (s.done ? ' done' : '')} onClick={() => !disabled && onSelect(s.id)} disabled={disabled} title={locked && !active ? 'Locked while exporting' : s.hint} aria-current={active ? 'step' : undefined}>
